  ...
  const sendOrderConfirmationEmail = async (order) => {
    try {
      const emailParams = {
        ...
        payment_method: paymentMethods.find(m => m.id === order.paymentMethod)?.name,
  ...
  const handlePlaceOrder = async (e) => {
    ...
    try {
      ...
      if (emailSent) {
        ...
        alert('Order placed but email could not be sent. Please save your order details.');
  ...
  const OrderSuccess = () => (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center">
      ...
      >
        ...
        <p className="text-gray-600 mb-2">Order ID: <span className="font-semibold">{orderDetails?.orderId}</span></p>
        <p className="text-gray-600 mb-6">Confirmation email has been sent to {orderDetails?.email}</p>
                ...
                {paymentMethods.map((method) => (
                  <div key={method.id} className="flex items-start gap-4">
                    ...
                    <label htmlFor={method.id} className="flex-1 cursor-pointer">
                      ...
                      {formData.paymentMethod === method.id && method.id !== 'cash' && (
                        <motion.div
                          ...
                          {(method.id === 'bkash' || method.id === 'nagad') && (
                            <>
                              <div className="font-semibold text-gray-800 mb-2">
                                {method.name} Number: 
                                <span className="text-[#940000] ml-2 text-lg">{method.number}</span>
                              </div>
                              <div className="mb-3">
    ...
    <div className="mt-6 p-5 bg-gradient-to-br from-gray-50 to-white rounded-xl border border-gray-200 shadow-sm">
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-100">
          <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center">
            <span className="text-blue-600 font-bold text-sm">ব</span>
          </div>
          <div className="flex-1">
            <p className="text-xs text-gray-500 uppercase">bank name</p>
            <p className="font-semibold text-gray-800">Agrani Bank Limited</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-100">
          ...
          <div className="flex-1">
            <p className="text-xs text-gray-500 uppercase">ccount holder name</p>
            <p className="font-semibold text-gray-800">MD MASUD RANA</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-100">
          ...
          <div className="flex-1">
            ...
            <p className="font-semibold text-gray-800 font-mono">0200020330838</p>
          </div>
          <button
            type="button"
            onClick={() => {
              navigator.clipboard.writeText('0200020330838');
        ...
        <div className="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-100">
          ...
          <div className="flex-1">
            ...
            <p className="font-semibold text-gray-800">NALUA BAZAR BR 0039</p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-5 pt-4 border-t border-gray-300">
        <div className="flex items-start gap-2">
          <svg className="w-5 h-5 text-[#940000] mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <div>
            <h5 className="font-medium text-gray-700 mb-1">Payment Instructions:</h5>
            <ul className="space-y-1 text-sm text-gray-600">
              ...
              <li className="flex items-center gap-2">
                <span className="w-1.5 h-1.5 bg-[#940000] rounded-full"></span>
                Enter transaction ID/receipt number above
              </li>
              <li className="flex items-center gap-2">
                <span className="w-1.5 h-1.5 bg-[#940000] rounded-full"></span>
